...
import { slash, getMappedClassName } from '../../../../util'
import { classNameTask } from '../../../../util/code'
...
export function method (methodInfo: MethodInfo) {
  ...
  if (sig.endsWith('L' + clsInfo.obfName + ';') && methodInfo.flags.static && !methodInfo.obfName.startsWith('value')) return 'get'
...
function clinit (methodInfo: MethodInfo) {
  ...
  info.scheduleTask(classNameTask({
    Block: CLASS.BLOCK,
    BlockPos: CLASS.BLOCK_POS,
    Difficulty: CLASS.DIFFICULTY,
    Facing: CLASS.FACING,
    ItemStack: CLASS.ITEM_STACK,
    NBTCompound: CLASS.NBT_COMPOUND,
    TextComponent: CLASS.TEXT_COMPONENT
  }, ({ Block, BlockPos, Difficulty, Facing, ItemStack, NBTCompound, TextComponent }) => {
        ...
        for (const direction of [state.clientbound, state.serverbound]) {
          ...
          for (const packet of direction) {
            ...
            packet.fields = packet.fields.replace(/L([a-z$\d]{1,6})(?=[;<])/g, (full, obfName) => {
              ...
              return `L${slash(getMappedClassName(info.class[obfName]))}`
            })
          }
        }
      }
                  ...
                  case 'play': {
                    ...
                    switch (descr) {
                      ...
                      case 'clientbound,I,F':
                        fieldsDeobf = ['action', 'value']
                        return pkg + '.ChangeGameState' + dirShort
                    }
                    if (BlockPos && Facing && descr === `clientbound,I,Ljava/util/UUID;,L${BlockPos};,L${Facing};,I`) {
                      fieldsDeobf = ['entityId', 'uuid', 'pos', 'facing', 'motive']
                      return pkg + '.SpawnPainting' + dirShort
                    }
                    if (BlockPos && descr === `clientbound,I,L${BlockPos};,I`) {
                      fieldsDeobf = ['entityId', 'pos', 'progress']
                      return pkg + '.BlockBreakProgress' + dirShort
                    }
                    if (BlockPos && NBTCompound && descr === `clientbound,L${BlockPos};,I,L${NBTCompound};`) {
                      fieldsDeobf = ['pos', 'action', 'data']
                      return pkg + '.UpdateBlockEntity' + dirShort
                    }
                    if (BlockPos && Block && descr === `clientbound,L${BlockPos};,2*I,L${Block};`) {
                      fieldsDeobf = ['pos', 'action', 'param', 'block']
                      return pkg + '.BlockAction' + dirShort
                    }
                    if (Difficulty && descr === `clientbound,L${Difficulty};,Z`) {
                      fieldsDeobf = ['difficulty', 'locked']
                      return pkg + '.SetDifficulty' + dirShort
                    }
                    if (TextComponent && descr === `clientbound,I,Ljava/lang/String;,L${TextComponent};,2*I`) {
                      fieldsDeobf = ['id', 'type', 'title', 'slots', 'horseId']
                      return pkg + '.OpenWindow' + dirShort
                    }
                    if (ItemStack && descr === `clientbound,I,Ljava/util/List<L${ItemStack};>;`) {
                      fieldsDeobf = ['windowId', 'slots']
                      return pkg + '.SetInventoryContents' + dirShort
                    }
                    if (ItemStack && descr === `clientbound,2*I,L${ItemStack};`) {
                      fieldsDeobf = ['windowId', 'slot', 'item']
                      return pkg + '.UpdateInventorySlot' + dirShort
                    }
                    break
                  }
                  case 'status': {
